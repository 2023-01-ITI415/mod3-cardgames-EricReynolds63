...
public class Card : MonoBehaviour {
    ...
    /// <summary>
    /// Creates this CardÅfs visuals based on suit and rank.
    /// Note that this method assumes it will be passed a valid suit and rank.
    /// </summary>
    /// <param name="eSuit">The suit of the card (e.g., ÅfCÅf)</param>
    ...
    // An Euler rotation of 180Åã around the Z-axis will flip sprites upside down
    ...
    private void AddDecorators() {                         
        ...
        foreach (JsonPip pip in JsonParseDeck.DECORATORS) {
            ...
            // Name this GameObject so itÅfs easy to find in the Hierarchy
            _tGO.name = pip.type;
    ...
    private void AddPips() {
        ...
        foreach (JsonPip pip in def.pips) {
            ...
            _tSRend.sortingOrder = 1;
            // Add this to the CardÅfs list of pips
            pipGOs.Add(_tGO);
        }
    }
    ...
    private void AddFace() {
        if ( def.face == "" )
            return;// No need to run if this isnÅft a face card
    ...
    void PopulateSpriteRenderers() {
        // If weÅfve already populated spriteRenderers, just return.
        if (spriteRenderers != null) return;
        // GetComponentsInChildren is slow, but weÅfre only doing it once per card
    ...
    public void SetSortingOrder(int sOrd) {
        ...
        foreach (SpriteRenderer srend in spriteRenderers) {
            if (srend.gameObject == this.gameObject) {
                // If the gameObject is this.gameObject, itÅfs the card face
                srend.sortingOrder = sOrd;  // Set its order to sOrd
            } else if (srend.gameObject.name == "back") {
                // If itÅfs the back, set it to the highest layer
                srend.sortingOrder = sOrd + 2;
            } else {
                // If itÅfs anything else, put it in between.
    ...
    public bool AdjacentTo(Card otherCard, bool wrap=true) {
        ...
        // If either card is face-down, itÅfs not a valid match.
