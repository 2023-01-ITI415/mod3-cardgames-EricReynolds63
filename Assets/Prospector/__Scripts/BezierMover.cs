...
public class BezierMover : MonoBehaviour {
    ...
    public void Init( List<Vector3> pts, float timeD = 1, float timeS = 0 ) {
        ...
        // And if thereÅfs only that one point, then just callback
        if ( pts.Count == 1 ) {
    ...
    void Update() {
        ...
        if ( u < 0 ) {  // If u < 0, then we shouldnÅft move yet.
            state = eState.pre;
        } else {
            if ( u < 1 ) {  // 0 <= u <1, which means this is active and moving
                ...
                uCurved = 1;// Set uCurved=1 so we donÅft overshoot
                state = eState.post;
                completionEvent.Invoke();
            }
