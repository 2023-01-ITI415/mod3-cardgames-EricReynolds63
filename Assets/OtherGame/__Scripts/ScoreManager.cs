	...
	public class ScoreManager : MonoBehaviour {
		...
		void Tally( eScoreEvent evt ) {
			switch ( evt ) {
				...
				// These same things need to happen whether itÅfs a draw, win, or loss
			...
			switch ( evt ) {
				case eScoreEvent.gameWin:
					...
					// If itÅfs a win, add the score to the next round
					// static fields are NOT reset by SceneManager.LoadScene()
					SCORE_FROM_PREV_ROUND = score;
 
					// If itÅfs higher than the HIGH_SCORE, update HIGH_SCORE
					if (HIGH_SCORE <= score) {
				...
				case eScoreEvent.gameLoss:
					// If itÅfs a loss, check against the high score
					if (HIGH_SCORE <= score) {
		...
		void FloatingScoreHandler(eScoreEvent evt) {
			...
			case eScoreEvent.mine:// Remove a mine card
				...
				if ( fsFirstInRun == null ) {
					// Set it to stick around when itÅfs done
					fsFirstInRun = fs;
					fs.fontSizes[2] = 48;
				} else {
			...
			// Same things need to happen whether itÅfs a draw, a win, or a loss
			case eScoreEvent.draw:// Drawing a card
			case eScoreEvent.gameWin:// Won the round
			case eScoreEvent.gameLoss:// Lost the round
				// Add fsFirstInRun to the ScoreBoard score
				if ( fsFirstInRun != null ) {
					// Create points for the B?zier curve
					fsPts = new List<Vector2>();
					fsPts.Add( fsPosRun );
					fsPts.Add( fsPosMid2 );
					fsPts.Add( fsPosEnd );
					...
					fsFirstInRun = null;  // Clear fsFirstInRun so itÅfs created again
				}
